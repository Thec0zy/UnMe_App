...
namespace UnMe_App
{
    public partial class Goal2 : Form
    {
        public Goal2()
        {
            InitializeComponent();
        ...
        {
            // Get user input
            string goalName = txtGoal2.Text; // The name of the goal
            int goalTime;

            // Check if the user selected a time from the ComboBox
            if (cboGoal2.SelectedItem != null)
            {
                goalTime = int.Parse(cboGoal2.SelectedItem.ToString());

                // Save the goal to the database
                SaveGoal(2, goalName, goalTime); // Goal number 1
                MessageBox.Show("Goal 2 saved successfully!"); // Confirmation message
            }
            else
            {
