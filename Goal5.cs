...
namespace UnMe_App
{
    public partial class Goal5 : Form
    {
        public Goal5()
        {
            InitializeComponent();
        ...
        {
            // Get user input
            string goalName = txtGoal5.Text; // The name of the goal
            int goalTime;

            // Check if the user selected a time from the ComboBox
            if (cboGoal5.SelectedItem != null)
            {
                goalTime = int.Parse(cboGoal5.SelectedItem.ToString());

                // Save the goal to the database
                SaveGoal(5, goalName, goalTime); // Goal number 1
                MessageBox.Show("Goal 5 saved successfully!"); // Confirmation message
            }
            else
            {
